...
export default function SalesRegionDetails() {
  const navigate = useNavigate();
  const { region } = useParams();
  const decodedRegion = useMemo(() => decodeURIComponent(region || ""), [region]);
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setErr("");
      try {
        const url = `${base}/api/locations/regions/${encodeURIComponent(decodedRegion)}/summary?locationScope=${scope}`;
        const res = await fetch(url);
        if (!res.ok) throw new Error(`API error: ${res.status}`);
        const json = await res.json();
        if (!ignore) setData(json);
      } catch (e) {
        if (!ignore) setErr(e?.message || "Failed to load region summary");
      } finally {
        if (!ignore) setLoading(false);
      }
    }
    if (decodedRegion) run();
    return () => {
      ignore = true;
    };
  }, [decodedRegion, scope]);
  ...
  const viewOrgs = (extra = {}) => {
    navigate(
      buildOrgsUrl({
        SALES_REGION: decodedRegion,
        locationScope: extra.locationScope ?? scope,
        ...(extra.SERVICES ? { SERVICES: extra.SERVICES } : {}),
        ...(extra.CONTENT_TYPES ? { CONTENT_TYPES: extra.CONTENT_TYPES } : {}),
        ...(extra.ORG_FUNCTIONAL_TYPE ? { ORG_FUNCTIONAL_TYPE: extra.ORG_FUNCTIONAL_TYPE } : {}),
      })
    );
  };
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            <Pill kind="soft" title="Scope controls whether country/city presence counts across any production location or only HQ">
              Scope:
              <button
                type="button"
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <Pill kind="solid" onClick={() => viewOrgs({ locationScope: scope })} title="Open filtered organizations list">
        ...
        {data ? (
          <>
            ...
            <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 10 }}>
              ...
              <Metric label="Countries" value={data?.totals?.countries ?? 0} />
            ...
            <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: 12 }}>
              ...
              <Card
                title="Countries"
                subtitle="Click Details to open a country profile. Click View to open filtered orgs."
              >
                        ...
                        <tr key={c.name} style={{ borderTop: "1px solid rgba(0,0,0,0.06)" }}>
                          ...
                          <td style={{ padding: "10px 6px", display: "flex", gap: 8, justifyContent: "flex-end", flexWrap: "wrap" }}>
                            <Pill
                              onClick={() =>
                                navigate(
                                  `/participants/organizations/production-locations/countries/${encodeURIComponent(c.name)}`
                                )
                              }
                            ...
                            <Pill onClick={() => navigate(buildOrgsUrl({ GEONAME_COUNTRY_NAME: c.name, SALES_REGION: decodedRegion, locationScope: scope }))}>
                              View →
                            </Pill>
                          </td>
                        </tr>
      ...
      hasSelections ? (
        ...
        >
          <div style={{ maxWidth: 1180, margin: "0 auto" }}>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                <Pill kind="solid">{decodedRegion}</Pill>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                <Pill
                  ...
                  onClick={() => {
                    ...
                    navigate(buildOrgsUrl({ SALES_REGION: decodedRegion, ...extra, locationScope: "all" }));
                  }}
                ...
                <Pill
                  ...
                  onClick={() => {
                    const extra = buildFacetParams();
                    navigate(buildOrgsUrl({ SALES_REGION: decodedRegion, ...extra, locationScope: "hq" }));
                  }}
