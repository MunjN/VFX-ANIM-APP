...
//         const r = await fetch("/api/orgs/services/counts");
//         if (!r.ok) throw new Error(`HTTP ${r.status}`);
//         const j = await r.json();
//         if (!alive) return;
...
//           const rr = await fetch(`/api/orgs?page=${page}&pageSize=${pageSize}`);
//           if (!rr.ok) throw new Error(`HTTP ${rr.status} while loading orgs`);
//           const jj = await rr.json();
//           const rows = Array.isArray(jj?.orgs) ? jj.orgs : Array.isArray(jj?.rows) ? jj.rows : [];
